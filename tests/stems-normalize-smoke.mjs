...
assert.match(handler, /def melband_quality_status/);
assert.match(handler, /MELBAND_QUALITY_STATUS/);
assert.match(handler, /if quality_status == "hold":/);
assert.match(handler, /MelBand is on quality hold; used Demucs/);
...
assert.match(handler, /def worker_capabilities/);
assert.match(handler, /"action": "capabilities"/);
assert.match(handler, /"canonicalStems": \["vocals", "bass", "drums", "other"\]/);
assert.match(handler, /"melbandEnabled": env_enabled\("ENABLE_MELBAND", False\)/);
assert.match(handler, /"melbandQualityStatus": melband_quality_status\(\)/);
assert.match(handler, /if str\(payload\.get\("action"\) or ""\)\.lower\(\) == "capabilities":/);
...
const provenanceUi = fs.readFileSync(new URL('../js/app-separation-provenance.js', import.meta.url), 'utf8');
assert.match(provenanceUi, /function melbandQualityStatus\(\)/);
assert.match(provenanceUi, /\['hold', 'candidate', 'approved'\]/);
assert.match(provenanceUi, /function constrainCandidateModel\(stem, plan\)/);
assert.match(provenanceUi, /const scoreCap = hold \? 55 : 79/);
assert.match(provenanceUi, /const maxWet = hold \? 0\.10 : 0\.24/);
assert.match(provenanceUi, /const operationFactor = hold \? 0\.45 : 0\.72/);
assert.match(provenanceUi, /Benchmark-safe max/);
assert.match(provenanceUi, /model trust \$\{trust\.status\}/);
assert.match(provenanceUi, /buildStemPlansWithModelTrust/);
