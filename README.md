...
3. Choose **Quick Master** (stereo release master + Original vs Master A/B) or **Forensic Fix** (timeline + opt-in stems + targeted repair). Quick Master skips Isolate / Confirm / Repair, but it still applies evidence-bounded stereo EQ for measured #1 issues (sub-bass accumulation, dark top) before loudness — it does not wait for Forensic stems. When the scan finds a buried/masked lead, Forensic is recommended and writes time-sliced vocal rides on buried timeline windows (a verse that ducks under other gets a ride; a forward chorus stays put). It remasures and repeats until those windows clear or a hard cap stops it — it does not treat a song-length +2 dB (or a bigger one-shot) as done. Skipping stems is honest: a stereo master cannot unbury the vocal.
