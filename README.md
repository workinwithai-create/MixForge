...
3. Choose **Quick Master** (stereo release master + Original vs Master A/B) or **Forensic Fix** (timeline + opt-in stems + targeted repair). Quick Master skips Isolate / Confirm / Repair, but it still applies evidence-bounded stereo EQ for measured #1 issues (sub-bass accumulation, dark top) before loudness — it does not wait for Forensic stems. When the scan finds a buried/masked lead, Forensic is recommended and defaults to isolating the vocal and raising it in the mix (level/balance only). Skipping stems is honest: a stereo master cannot unbury the vocal.
