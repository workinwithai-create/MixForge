...
const MixForgeHubClient = {
  ...
  async refresh() {
    ...
    let local = null;
    try {
      const hubRes = await mfFetchJson(MF_HUB_ENTITLEMENTS_URL, {
        method: 'GET',
        credentials: 'include',
  ...
  async startCheckout(lookupKey = 'mix-monthly') {
    ...
    try {
      ...
      if (result.status === 401) {
        if (globalThis.location) globalThis.location.href = mfHubLoginUrl(returnTo);
        return { ok: false, reason: 'login' };
      }
      if (!result.ok || !result.payload?.url) {
        if (globalThis.location) globalThis.location.href = `${MF_HUB_ORIGIN}/#pricing`;
...
if (typeof document !== 'undefined') {
  const start = () => { void MixForgeHubClient.refresh(); };
