...
function mfForensicAudit(metrics, notes, targetLufs) {
  ...
  const masking = band(metrics, 'Low-mids') - band(metrics, 'Presence');
  if (masking > 14) add('medium', 'Lead-band masking condition', `Center presence is ${masking.toFixed(1)} dB below dominant low-mids.`, 'Lyrics or lead articulation may recede on small speakers.', [{ stem:'vocals', likelihood: mfConfidence(55 + masking) }, { stem:'other', likelihood: mfConfidence(45 + masking*.9) }], 'Measure vocal intelligibility against residual other occupancy after isolation.', 83);
  const sideMud = band(metrics, 'Low-mids', true) - band(metrics, 'Mids', true);
  if (sideMud > 8) add('medium', 'Wide low-mid accumulation', `Side-channel low-mids are ${sideMud.toFixed(1)} dB above side mids.`, 'The mix may feel wide but indistinct.', [{ stem:'other', likelihood: mfConfidence(58 + sideMud*2) }], 'Isolate residual other (guitars/keys/ambience share this Demucs bucket); confirm with reconstruction-safe comparison.', 81);
  const flare = metrics.sibilance.p95Db - metrics.sibilance.medianDb;
  if (flare > 7 && metrics.sibilance.flares > metrics.sibilance.frames*.05) add('medium', 'Intermittent high-frequency flares', `${metrics.sibilance.flares} events rise ${flare.toFixed(1)} dB above the normal top-end envelope.`, 'Sibilance, pick attack or cymbals may become fatiguing.', [{ stem:'vocals', likelihood: mfConfidence(50 + flare*2) }, { stem:'drums', likelihood: mfConfidence(35 + flare) }, { stem:'other', likelihood: mfConfidence(30 + flare*1.5) }], 'Separate likely contributors and identify which stem contains the events before applying dynamic control.', 78);
  if (metrics.correlation < .15) add('high', 'Mono compatibility risk', `Stereo correlation is ${metrics.correlation.toFixed(2)}.`, 'Important elements may cancel in mono.', [{ stem:'other', likelihood: 70 }, { stem:'drums', likelihood: 48 }], 'Run residual-other polarity and reconstruction checks before any width processing.', 91);
  ...
  const priority = ['vocals','bass','drums','other'];
...
renderStemPlans=function(){
  ...
  for(const [stem,plan] of Object.entries(state.stemPlans)){
    const card=mfEl('article','stem-card forensic-stem'); const head=mfEl('div','stem-head'); head.append(mfEl('h3','',stem),mfEl('span',`badge quality-${plan.quality.risk}`,`Leakage/fit heuristic ${plan.quality.score}%`)); card.append(head,mfEl('p','stem-guidance',plan.quality.guidance));
